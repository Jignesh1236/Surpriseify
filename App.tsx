...
const App: React.FC = () => {
  ...
  const reset = () => {
    setData({ vibe: Vibe.LOVE, recipientName: '', senderName: '', message: '', photo: '' });
    setIsUnveiled(false);
        ...
        <main className="w-full max-w-6xl mx-auto px-4 md:px-6 py-12 md:py-24 flex flex-col items-center">
          <header className="text-center mb-16 md:mb-20 space-y-4">
            <div className="inline-block px-4 py-1 border border-white/10 rounded-full mb-4">
              <span className="text-[9px] md:text-[10px] uppercase tracking-[0.5em] text-white/50">Version 2.8 • Premium Experience</span>
            </div>
          ...
          <div className="w-full glass-card rounded-[40px] md:rounded-[50px] p-8 md:p-14 space-y-10 md:space-y-12 relative overflow-hidden">
            ...
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="group space-y-2">
                  <label className="text-[9px] md:text-[10px] uppercase tracking-[0.3em] text-white/20 font-bold ml-1">Personal Photo (Optional)</label>
                  <div 
                    onClick={() => fileInputRef.current?.click()}
                    className="w-full h-32 md:h-full border-2 border-dashed border-white/10 rounded-3xl flex flex-col items-center justify-center cursor-pointer hover:border-white/30 hover:bg-white/5 transition-all relative overflow-hidden group"
                  >
                    {data.photo ? (
                      <>
                        <img src={data.photo} className="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:opacity-80 transition-opacity" alt="Preview" />
                        <div className="relative z-10 text-[9px] font-bold uppercase tracking-widest text-white drop-shadow-lg">Change Photo</div>
                        <button 
                          onClick={(e) => { e.stopPropagation(); setData({...data, photo: ''}); }}
                          className="absolute top-2 right-2 z-20 bg-black/50 p-2 rounded-full hover:bg-red-500/80 transition-colors"
                        >
                          ✕
                        </button>
                      </>
                      ...
                      <>
                        ...
                        <div className="text-[9px] font-bold uppercase tracking-widest text-white/30">Upload Moment</div>
                      </>
                    )}
                    <input 
                      type="file" 
                     ...
                     {data.photo && (
                       <div className="relative mb-6">
                         <div className="w-52 h-52 md:w-64 md:h-64 rounded-full border-8 border-red-500/20 p-2 overflow-hidden shadow-[0_0_80px_rgba(255,45,85,0.5)] animate-float">
                            <img src={data.photo} className="w-full h-full object-cover rounded-full" alt="Memory" />
                         </div>
                         <div className="absolute -bottom-4 -right-4 text-4xl animate-pulse">💖</div>
                       </div>
                     )}

                     <div className="text-8xl md:text-[15rem] opacity-5 absolute -top-24 md:-top-40 left-1/2 -translate-x-1/2 select-none animate-[heartbeat_2s_infinite]">❤️</div>
                   ...
                   <div className="relative w-full max-w-6xl py-12 flex flex-col md:flex-row items-center justify-center gap-12 md:gap-20">
                      ...
                      {data.photo && (
                        <div className="w-64 h-80 md:w-80 md:h-[30rem] border-4 border-[#d4af37] p-2 bg-neutral-900 shadow-[20px_20px_60px_rgba(0,0,0,0.8)] transform -rotate-3 hover:rotate-0 transition-transform duration-700">
                           <img src={data.photo} className="w-full h-full object-cover opacity-80" alt="Eternal Moment" />
                        </div>
                      )}
                      ...
                      <div className="glass-card rounded-none border-0 p-6 md:p-12 space-y-12 md:space-y-16 flex flex-col items-center md:items-start text-center md:text-left flex-1">
                        <div className="luxury-text elegant-font text-base md:text-2xl uppercase tracking-[0.4em] font-bold">The Promise</div>
                        ...
                        {!showCelebrate ? (
                          ...
                          <div className="animate-reveal py-8 w-full">
                             ...
                             <div className="mt-4 flex justify-center md:justify-start gap-4 text-3xl md:text-4xl">✨ 🕊️ ✨</div>
                          </div>
                        )}
                      </div>
                   </div>
                      ...
                      {data.photo && (
                        <div className="absolute top-0 right-0 md:top-10 md:right-10 w-40 h-40 md:w-64 md:h-64 opacity-40 blur-[2px] hover:opacity-100 hover:blur-0 transition-all duration-1000">
                           <div className="w-full h-full rounded-[40%_60%_70%_30%_/_40%_50%_60%_40%] overflow-hidden border-2 border-blue-500/30">
                              <img src={data.photo} className="w-full h-full object-cover grayscale" alt="Reflections" />
                           </div>
                        </div>
                      )}
                        ...
                        {data.photo && (
                          <div className="absolute -bottom-10 right-0 md:-right-10 w-48 h-56 md:w-64 md:h-72 bg-white p-2 rotate-12 shadow-[15px_15px_0px_#af52de] transform hover:scale-110 transition-transform duration-500 z-20">
                             <div className="w-full h-[85%] overflow-hidden">
                               <img src={data.photo} className="w-full h-full object-cover" alt="The Duo" />
                             </div>
                             <div className="text-black font-black text-[10px] mt-2 tracking-tighter uppercase">STAY WILD / {new Date().getFullYear()}</div>
                          </div>
                        )}
                      </div>
                   ...
                   <div className="relative w-full max-w-5xl py-12 flex flex-col items-center">
                     ...
                     {data.photo && (
                        <div className="absolute inset-0 flex items-center justify-center opacity-20 -z-10 blur-xl scale-150 overflow-hidden">
                           <img src={data.photo} className="w-full h-full object-cover rounded-full" alt="BG" />
                        </div>
                     )}
                        ...
                        {data.photo && (
                           <div className="relative w-48 h-48 md:w-64 md:h-64 mb-4">
                              ...
                              <div className="w-full h-full rounded-full border-4 border-white overflow-hidden shadow-[0_0_100px_rgba(255,223,0,0.5)]">
                                 <img src={data.photo} className="w-full h-full object-cover" alt="The Icon" />
                              </div>
